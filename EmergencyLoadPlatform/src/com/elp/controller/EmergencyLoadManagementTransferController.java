...
@Controller
@RequestMapping("emergencyloadmanagement-transfer")
public class EmergencyLoadManagementTransferController implements ServletContextAware{
	
	@Autowired
	private ServletContext servletContext;
	private static final Logger logger = Logger.getLogger(EmergencyLoadManagementTransferController.class);
	...
	@RequestMapping(method = RequestMethod.GET)
		...
		ModelAndView page = new ModelAndView("emergencyloadmanagement-transfer");
		...
		if (null == isSession.getAttribute("username")) {		
				 ...
				 {		
					...
					List<CreditTransferModel> creditlist = dao.getTransferlist(cid,session);
    ...
    public @ResponseBody String reTransfer(
	...
	ApplicationContext  context = new ClassPathXmlApplicationContext("Spring-Customer.xml");
		...
		if(dao.resendTransfer(cid, session)){
			return "Transfer Request Successful";
		}else{
			return "Transfer Request Fail";
		}
