<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="utf-8" />

	
	
	<style type="text/css">
		* {
			margin: 0;
			padding: 0;
			outline: 0;
		}
		
		html, body{
			height: 100%;
			background: #F2F2F2;
			width:100%;
		}

		body, a{
			font: normal 16px Helvetica, Verdana, Geneva, sans-serif;
			color: #3F3F3F;		
		}
		
		.container{
			min-height: 100%;
			height: auto !important;
			height: 100%;
			margin: 0 auto -30px;
			width:100%;
			background-color: #f1f1f1;
		}
		
		.container:after{
			content: '';
			display: block;
			height: 30px;
			clear: both;
		}
		
		.footer{
			height: 12px;
			padding: 8px 0;
			background: #FFF;
			border-top: 1px solid #51C1F1;
			font-size: 12px;
			text-align: center;
			width:100%;
			margin-top: 80px;
		}

		
	</style>



		<link rel="stylesheet" href="css/istyle.css" />



	
	<link rel="stylesheet" href="css2/agenttable.css" />


	<link rel="stylesheet" href="css/font-awesome.css" />
<link rel="stylesheet" href="css/font-awesome.min.css" />
	<link rel="stylesheet" href="css/agent.css" />
	<link rel="stylesheet" href="css/agentlist.css" />
	<title>Circles</title>
</head>
<body>
	<div class="container">
		<div class="header clearfix">
			
			
			<div class="nav">
				
				<ul class="menu">
					<li><a href="salesmtd.html"><i class="fa fa-usd"></i>&nbsp;SALES</a></li>
					<li><a href="agent.html" class="selected"><i class="icon-agent" style="font-size:20px;"></i>&nbsp;AGENTS</a></li>		
					<li><a href="recent.html"><i class="fa fa-clock-o"></i>&nbsp;RECENT</a></li>	
							<li><a href="settings.html"><i class="fa fa-mobile-phone"></i>&nbsp;MOBILE</a></li>		
					<li><a href="index.html"><i class="fa fa-laptop"></i>&nbsp;Log out</a></li>
					
				</ul>
			</div><!-- End of Navigation -->
			
		</div><!-- End of Header -->

		<div class="application">
		<text ><a href="agent.html" >Applications</a></text>

		<text class="preactive" style="margin-left:15px;"><a href="agentsales.html">Sales</a></text>
		</div>


			

		
 <table cellspacing="0" cellpadding="0" border="0" style=" margin-left:auto; margin-right:auto; margin-top:50px;">
   <tbody>
                       <tr height="35">
                         <td height="35" width="120"> <a href="agentrecent.html">Thomas Salvani</a></td>
                         <td width="800px">
		<article class="module width_3_quarter">
			
			<div id="tab1" class="tab_content">
			<table class="tablesorter" > 
			<thead> 
				<tr > 
   					<th> </th>
    				<th>Jan</th> 
    				<th>Feb</th> 
    				<th>Mar </th> 
    				<th>Apr </th>
    				<th>May </th>
    				<th>Jun </th>
    				<th>Jul </th>
    				<th>Aug </th>
    				<th>Sep </th>
    				<th>Oct </th>
    				<th>Nov </th>
    				<th>Dec </th>
    				<th>Total </th>

				</tr> 
			</thead> 
			<tbody> 
				<tr class="colorth"> 
   					<td></td> 
    				<td>P150,000</td> 
    				<td>P150,000</td> 
    				<td>P150,000</td> 
    			  	<td>P150,000</td> 
    			  	<td>P150,000</td> 
    			  	<td>P150,000</td> 
    			  	<td>P150,000</td> 
    			  	<td>P150,000</td> 
    			  	<td>P150,000</td> 
    			  	<td>P150,000</td> 
					<td>P150,000</td> 
					<td>P150,000</td> 
					<td>P150,000</td> 
				</tr> 
				<tr class="colortd1"> 
   					<td></td> 
    				<td>P100,000</td> 
    				<td>P100,000</td> 
    				<td>P100,000</td>
    			  	<td>P100,000</td>
    				<td>P100,000</td>
    			  	<td>P100,000</td>
    			  	<td>P100,000</td>
    			  	<td>P100,000</td>
    			  	<td>P100,000</td>
    			 	<td>P100,000</td>
					<td>P100,000</td>
					<td>P100,000</td>
					<td>P100,000</td>
				</tr> 

				<tr class="colortd2"> 
   					<td></td> 
    				<td>P90,000</td> 
    				<td>P90,000</td> 
    				<td>P90,000</td> 
    			  	<td>P90,000</td> 
    			  	<td>P90,000</td> 
    			  	<td>P90,000</td> 
    			  	<td>P90,000</td> 
    			  	<td>P90,000</td> 
    			  	<td>P90,000</td> 
    			  	<td>P90,000</td> 
					<td>P90,000</td> 
					<td>P90,000</td> 
					<td>P90,000</td> 
				</tr> 
			
			</tbody> 
			</table>
		
		
		
			
		</div><!-- end of tab_container -->
		
		</article><!-- end of content manager article -->
</td>


<tr height="35">
                         <td height="35" width="106"><a href="agentrecent.html">Thomas Salvani</a></td></td>
                         <td width="800px">
		<article class="module width_3_quarter">
			
			<div id="tab1" class="tab_content">
			<table class="tablesorter" > 
			<thead> 
				<tr> 
   					<th> </th>
    				<th>Jan</th> 
    				<th>Feb</th> 
    				<th>Mar </th> 
    				<th>Apr </th>
    				<th>May </th>
    				<th>Jun </th>
    				<th>Jul </th>
    				<th>Aug </th>
    				<th>Sep </th>
    				<th>Oct </th>
    				<th>Nov </th>
    				<th>Dec </th>
    				<th>Total </th>

				</tr> 
			</thead> 
			<tbody> 
				<tr class="colorth"> 
   					<td></td> 
    				<td>P150,000</td> 
    				<td>P150,000</td> 
    				<td>P150,000</td> 
    			  	<td>P150,000</td> 
    			  	<td>P150,000</td> 
    			  	<td>P150,000</td> 
    			  	<td>P150,000</td> 
    			  	<td>P150,000</td> 
    			  	<td>P150,000</td> 
    			  	<td>P150,000</td> 
					<td>P150,000</td> 
					<td>P150,000</td> 
					<td>P150,000</td> 
				</tr> 
				<tr class="colortd1"> 
   					<td></td> 
    				<td>P100,000</td> 
    				<td>P100,000</td> 
    				<td>P100,000</td>
    			  	<td>P100,000</td>
    				<td>P100,000</td>
    			  	<td>P100,000</td>
    			  	<td>P100,000</td>
    			  	<td>P100,000</td>
    			  	<td>P100,000</td>
    			 	<td>P100,000</td>
					<td>P100,000</td>
					<td>P100,000</td>
					<td>P100,000</td>
				</tr> 

				<tr class="colortd2"> 
   					<td></td> 
    				<td>P90,000</td> 
    				<td>P90,000</td> 
    				<td>P90,000</td> 
    			  	<td>P90,000</td> 
    			  	<td>P90,000</td> 
    			  	<td>P90,000</td> 
    			  	<td>P90,000</td> 
    			  	<td>P90,000</td> 
    			  	<td>P90,000</td> 
    			  	<td>P90,000</td> 
					<td>P90,000</td> 
					<td>P90,000</td> 
					<td>P90,000</td> 
				</tr> 
			
			</tbody> 
			</table>
		
		
		
			
		</div><!-- end of tab_container -->
		
		</article><!-- end of content manager article -->
</td>




<tr height="35">
                         <td height="35" width="106"><a href="agentrecent.html">Thomas Salvani</a></td></td>
                         <td width="800px">
		<article class="module width_3_quarter">
			
			<div id="tab1" class="tab_content">
			<table class="tablesorter" > 
			<thead> 
				<tr> 
   					<th> </th>
    				<th>Jan</th> 
    				<th>Feb</th> 
    				<th>Mar </th> 
    				<th>Apr </th>
    				<th>May </th>
    				<th>Jun </th>
    				<th>Jul </th>
    				<th>Aug </th>
    				<th>Sep </th>
    				<th>Oct </th>
    				<th>Nov </th>
    				<th>Dec </th>
    				<th>Total </th>

				</tr> 
			</thead> 
			<tbody> 
				<tr class="colorth"> 
   					<td></td> 
    				<td>P150,000</td> 
    				<td>P150,000</td> 
    				<td>P150,000</td> 
    			  	<td>P150,000</td> 
    			  	<td>P150,000</td> 
    			  	<td>P150,000</td> 
    			  	<td>P150,000</td> 
    			  	<td>P150,000</td> 
    			  	<td>P150,000</td> 
    			  	<td>P150,000</td> 
					<td>P150,000</td> 
					<td>P150,000</td> 
					<td>P150,000</td> 
				</tr> 
				<tr class="colortd1"> 
   					<td></td> 
    				<td>P100,000</td> 
    				<td>P100,000</td> 
    				<td>P100,000</td>
    			  	<td>P100,000</td>
    				<td>P100,000</td>
    			  	<td>P100,000</td>
    			  	<td>P100,000</td>
    			  	<td>P100,000</td>
    			  	<td>P100,000</td>
    			 	<td>P100,000</td>
					<td>P100,000</td>
					<td>P100,000</td>
					<td>P100,000</td>
				</tr> 

				<tr class="colortd2"> 
   					<td></td> 
    				<td>P90,000</td> 
    				<td>P90,000</td> 
    				<td>P90,000</td> 
    			  	<td>P90,000</td> 
    			  	<td>P90,000</td> 
    			  	<td>P90,000</td> 
    			  	<td>P90,000</td> 
    			  	<td>P90,000</td> 
    			  	<td>P90,000</td> 
    			  	<td>P90,000</td> 
					<td>P90,000</td> 
					<td>P90,000</td> 
					<td>P90,000</td> 
				</tr> 
			
			</tbody> 
			</table>
		
		
		
			
		</div><!-- end of tab_container -->
		
		</article><!-- end of content manager article -->
</td>













	</tbody>

		</table>








	
	</div><!-- End of Container -->

	

	<div class="clearme"></div>
	<div class="footer">
		<p>&copy; 2013 Circles</p>
	</div>
</body>
</html>