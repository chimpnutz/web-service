...
@Controller
@RequestMapping("holidaymanagement-add")
public class HolidayManagementAddController implements ServletContextAware{
	
	@Autowired
	private ServletContext servletContext;
	private static final Logger logger = Logger.getLogger(HolidayManagementAddController.class);
	...
	@RequestMapping(method = RequestMethod.GET)
	 public ModelAndView topupView(HttpServletRequest request,HttpSession session) throws NamingException {
		...
		ModelAndView page = new ModelAndView("holidaymanagement-add");
		...
		if (null == isSession.getAttribute("username")) {		
				 ...
				 {		
					...
					page.addObject("holidaymanagementForm",new HolidayModel());
	...
	public ModelAndView addcompany(@ModelAttribute("holidaymanagementForm") HolidayModel model,  
            BindingResult result,HttpSession session,HttpServletRequest request) 
	{
		...
		if(dao.addHoliday(session, model) == 0){
