...
//			result = gson.toJson(user);
	...
	public Collection getReturnApplication(Application application){
		return jdbcTemplate.query("SELECT a.*,  c.* FROM application a  INNER JOIN product c ON a.product_id = c.product_id WHERE a.application_type IS NOT NULL AND a.status = ?" , new ApplicationMapper5(),3);
		
	}
	public Collection getEncodedApplication(Application application){
		return jdbcTemplate.query("SELECT a.*,  c.* FROM application a  INNER JOIN product c ON a.product_id = c.product_id WHERE a.application_type IS NOT NULL AND a.status = ?",new ApplicationMapper5(),1);
		
	}
	
	public Collection getOngoingApplication(Application application){
		return jdbcTemplate.query("SELECT a.*, c.* FROM application a  INNER JOIN product c ON a.product_id = c.product_id WHERE a.application_type IS NOT NULL AND a.status = ?", new ApplicationMapper5(),2);
