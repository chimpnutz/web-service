...
public class NetworkManagementController implements ServletContextAware{
	
	@Autowired
	private ServletContext servletContext;
	private static final Logger logger = Logger.getLogger(NetworkManagementController.class);
	...
	@RequestMapping(method = RequestMethod.GET)
	 public ModelAndView topupView(HttpServletRequest request,HttpSession session) throws NamingException {
		...
		ModelAndView page = new ModelAndView("networkmanagement");
		...
		if (null == isSession.getAttribute("username")) {		
				...
				return loginpage;	
				 ...
				 {		
					...
					List<NetworkManagementModel> company = dao.getCompany(session);
					Map companylist = dao.getcompanylist(session);

					
					page.addObject("usermanagementForm",new UserManagementModel());
					page.addObject("companylistdrop",companylist);
	...
	@RequestMapping(method = RequestMethod.GET,params={"addDSP"})
	 public ModelAndView viewDSP(HttpServletRequest request,HttpSession session) throws NamingException {
		...
		if (null == isSession.getAttribute("username")) {		
				...
				return loginpage;	
				 ...
				 {		
					...
					List<PermissionModel> permission = Ldao.getmodulePermission(isSession);
					List<NetworkManagementModel> company = dao.getCompany();
					Map companylist = dao.getcompanylist(session);
	...
	@RequestMapping(method = RequestMethod.GET,params={"addRetailer"})
	 public ModelAndView viewRetailer(HttpServletRequest request,HttpSession session) throws NamingException {
